<?php
// 重写tp6分页类
// BootstrapDetailed类放在app\common下，命名为BootstrapDetailed.php
// 在/app/provider.php中自定义
// 'think\Paginator' => 'app\common\BootstrapDetailed'
...
class BootstrapDetailed extends Paginator
{
    ...
    protected  function totalshow()
    {
        ...
        $totalhtml="<div style='margin-top:25px;'><span>共".$this->total."条记录&nbsp&nbsp第".$this->currentPage()."页/共".$this->lastPage()."页</span></div>";
        return $totalhtml;
  ...
  //跳转到哪页
    ...
    public function render()
    {
        if ($this->hasPages()) {
            if ($this->simple) {
                ...
                return sprintf(
                    '<ul class="paginator"> %s %s %s %s %s %s </ul>',
    ...
    {
        return '<li class="pageItem"><a href="' . htmlentities($url) . '">' . $page . '</a></li>';
    }
    ...
    {
        return '<li class="pageItemDisable"><span>' . $text . '</span></li>';
    }
    ...
    {
        return '<li class="pageItemActive"><span>' . $text . '</span></li>';
    }
